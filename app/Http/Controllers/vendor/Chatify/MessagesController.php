...
class MessagesController extends Controller
{
    ...
    public function index($id = null)
    {
         ...
         // Set user as active when they open chat
        //User::where('id', Auth::user()->id)->update(['active_status' => 1]);

        // Get the latest conversation ID
        $latestMessage = Message::where(function($q) {
        ...
        // Use latest conversation ID if no specific ID provided
        $id = $id ?? ($latestMessage ? ($latestMessage->from_id == Auth::user()->id ? $latestMessage->to_id : $latestMessage->from_id) : 0);

        return view('Chatify::pages.app', [
            'id' => $id,
            'messengerColor' => Auth::user()->messenger_color ?? Chatify::getFallbackColor(),
    ...
    public function getContacts(Request $request)
{
    if (Auth::user()->is_admin == 1) {
        // For admin: Get all users except self, ordered by newest first
   ...
   public function search(Request $request)
{
    ...
    if (Auth::user()->is_admin == 1) {
        ...
        // Regular users can only search admin accounts
