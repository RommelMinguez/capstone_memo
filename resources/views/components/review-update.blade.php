...
<div id="update-review" class="fixed hidden inset-0 h-screen w-screen bg-opacity-60 bg-black z-50">
    <div class="flex h-full justify-center items-center">
        <form action="/user/review/{{ $myReview->id }}" method="POST" id="create-review-form">

            @csrf
            @method('PATCH')
            <input type="hidden" name="rating" id="rating" value="{{ $myReview->rating }}">

            <div class="p-5 bg-[#FEF6E4] rounded-xl border shadow-md">
                <div class="flex justify-between items-center">
                    <div class="font-bold">Edit Review</div>
                    <div id="close-update-review" title="Close Update Form" class="cursor-pointer aspect-square w-7 rounded-full p-2 hover:bg-red-500 hover:fill-white fill-red-500">
                        <svg
                            class="w-full"
                ...
                <div class="pb-10">
                    ...
                    <textarea required name="comment" id="comment" cols="50" rows="5" placeholder="type here..." class="p-5 shadow-md border rounded-md">{{ $myReview->comment }}</textarea>
                </div>

                <div class="flex justify-end">
                    <button id="submit-review" type="button" class="py-2 px-5 text-white font-semibold border rounded-md bg-red-500 shadow-sm hover:bg-red-600 active:scale-95">SAVE POST</button>
                </div>
            </div>
        </form>
    </div>
