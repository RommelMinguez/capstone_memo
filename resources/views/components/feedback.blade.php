<div class="h-36 shadow-md border rounded-md px-10 pt-10 pb-5 mt-5">
    <div class="flex justify-between items-center">
        <div class="text-red-500 font-bold flex gap-2">
            <div class="aspect-square w-7 rounded-full overflow-hidden">
                <img src="{{ Storage::url($review->user->image_src) }}" alt="profile" class="object-cover">
            </div>
            <div>{{ Str::title($review->user->first_name.' '.$review->user->last_name) }}</div>
            <div class="flex gap-1 fill-[#f44336] ml-10">
                @php
        ...
        <div>{{ $review->updated_at->diffForHumans() }}</div>
    </div>
    <br>
    <p>{{ $review->comment }}</p>
</div>
