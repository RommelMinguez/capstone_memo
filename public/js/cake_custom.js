...
let display_pollinations = document.getElementById('prompt-result-pollinations').cloneNode(true);
let display_horde = document.getElementById('prompt-result-horde').cloneNode(true);
...
function selectGeneratedImage(imgEl) {
    ...
    imageIcon.children[1].children[0].children[0].children[0].textContent = 'Change Image';
...
generate_btn.addEventListener('click', function() {
    ...
    if (prompt_inp.textContent.trim() === '') {
        alert('The prompt is a required field and cannot be left empty. Please provide a valid input.');
        return;
    }
...
async function getPollinationAI(dsc, display) {
    try {
        const response = await fetch("https://image.pollinations.ai/prompt/" + dsc);
...
async function checkHordeStatus(id, dsc, display) {
    try {
        ...
        if (!response.ok) {
            const errorDetails = await response.json();
            throw new Error(`Error ${response.status}: ${errorDetails.message || errorDetails.error}`);
        }
...
imageInput.addEventListener('change', function() {
    ...
    if (file) {
        ...
        imageIcon.children[1].children[0].children[0].children[0].textContent = 'Upload Image';
...
const previewPlaceholder = document.getElementById("preview-placeholder").cloneNode(true);

// Use DataTransfer to manage the input files
const dataTransfer = new DataTransfer();

const imageClasses = ["w-24", "h-24", "object-cover", "rounded-lg", "cursor-pointer"];
