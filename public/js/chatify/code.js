...
activeStatusChannel.bind("pusher:member_added", function (member) {
    ...
    contactElement.find(".activeStatus").remove();
    contactElement.find(".avatar").before(activeStatusCircle());
});
...
// Call getContacts immediately when page loads
...
function updateContactItem(user_id) {
    if (user_id != auth_id) {
        $.ajax({
            ...
            dataType: "JSON",
            success: (data) => {
                // Get the existing contact element
                ...
                const hasActiveStatus =
                    existingContact.find(".activeStatus").length > 0;

                // Remove existing contact item
                existingContact.remove();

                // Add updated contact item
                if (data.contactItem) {
                    const newContact = $(data.contactItem);

                    // Only add active status if it existed before
                    if (!hasActiveStatus) {
                        newContact.find(".activeStatus").remove();
                    }

                    $(".listOfContacts").prepend(newContact);
                }
...
function getContacts() {
    // Reset flags to ensure initial load
